...
//     await fetch('https://locations.logging.do' + pathname + '?' + new URLSearchParams(this.objects).toString())
    ...
    return new Response(JSON.stringify({ worker: this.locations.find(loc => loc.iata == req.cf.colo), durable: this.objects[colo], all: this.objects }, null, 2), { headers: { 'content-type': 'application/json; charset=utf-8' } })
  }
}
